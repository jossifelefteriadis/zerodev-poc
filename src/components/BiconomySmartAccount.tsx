...
//       // Predict (works across SDK shapes)
//       const predictedAddr =
//         (await (account as any).addressOn?.(optimismSepolia)) ??
//         (await (account as any).addressOn?.({ chain: optimismSepolia })) ??
//         (await (account as any).addressOn?.(optimismSepolia.id));

//       if (!predictedAddr) {
//         throw new Error(
//           'Could not resolve predicted smart account address on OP Sepolia.'
//         );
//       }
//       setPredicted(predictedAddr as `0x${string}`);
...
//       // Try to surface all possible identifiers
//       const maybeMeeHash =
//         (result as any)?.meeHash ??
//         (result as any)?.hash ?? // some older shapes
//         null;
...
//       // Sometimes receipts come back immediately
//       const maybeReceipts =
...
//       const res = await meeClient.waitForTransactionReceipts({ meeHash });
//       // res could be an array of receipts
//       setReceipts(res as any[]);
//       if (Array.isArray(res) && res[0]?.transactionHash) {
//         setTxHash(res[0].transactionHash);
//       }
//     } catch (e: any) {
//       setError(e?.message ?? String(e));
//     } finally {
...
//               <b>Predicted SA:</b>
//               <a
//                 href={`https://sepolia-optimism.etherscan.io/address/${predicted}`}
//                 target='_blank'
//                 rel='noreferrer'
//               >
//                 {predicted}
...
//               <summary className='cursor-pointer'>Receipts</summary>
